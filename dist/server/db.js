 ...
 */
export async function getNewsItems(category) {
    if (!connection)
        await getConnection();
    try {
        let query = rethinkdb.db(config.db).table('news');
...
export async function saveNewsItems(items) {
    ...
    try {
        ...
        if (itemsToUpdate.length > 0) {
            await Promise.all(itemsToUpdate.map(item => rethinkdb.db(config.db)
                .table('news')
                .get(item.id)
                .update(item)
                .run(connection)));
 ...
 */
export async function deleteOldNewsItems(olderThan) {
    if (!connection)
        await getConnection();
    try {
        const result = await rethinkdb.db(config.db)
