 ...
 */
export function checkForTimeBasedDiscoveries(allAvailableItems, discoveredItemIds, showingDiscovery) {
    if (!browser)
 ...
 */
export async function discoverNewItem(availableItems, currentItems, discoveredItemIds, seed) {
    ...
    const newerItems = availableItems.filter(item => new Date(item.publishDate).getTime() > newestCurrentDate);
    ...
    if (itemsToChooseFrom.length === 0) {
        ...
        itemsToChooseFrom = availableItems.filter(item => !currentItems.some(currentItem => currentItem.id === item.id));
    ...
    itemsToChooseFrom = itemsToChooseFrom.sort((a, b) => new Date(b.publishDate).getTime() - new Date(a.publishDate).getTime());
