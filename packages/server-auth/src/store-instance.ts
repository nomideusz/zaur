...
export function getStoreDb(): DatabaseSync {
	...
	const dbPath = process.env.STORE_DB_PATH?.trim() || DEFAULT_DB_PATH;
	db = openStoreDb(dbPath);

	// Legacy JSON store import — honours SESSION_STORE_PATH the old code used.
	const legacyPath = process.env.SESSION_STORE_PATH?.trim() || LEGACY_SESSIONS_JSON;
	try {
	...
	const linkedAccountsPath =
		process.env.LINKED_ACCOUNTS_STORE_PATH?.trim() || LEGACY_LINKED_ACCOUNTS_JSON;
	try {
