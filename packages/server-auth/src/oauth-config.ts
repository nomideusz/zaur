...
export function isStalwartOauthEnabled(): boolean {
	return (
		process.env.STALWART_OAUTH_ENABLED === 'true' &&
		Boolean(process.env.STALWART_OAUTH_CLIENT_ID?.trim()) &&
		Boolean(process.env.STALWART_OAUTH_ISSUER_URL?.trim())
	);
}
...
export function isPasswordLoginEnabled(): boolean {
	const explicit = parseBool(process.env.STALWART_PASSWORD_LOGIN_ROLLBACK_ENABLED);
	if (explicit !== undefined) return explicit;
	return process.env.STALWART_OAUTH_ENABLED?.trim().toLowerCase() !== 'true';
}
...
export function getStalwartOauthClientId(): string {
	const clientId = process.env.STALWART_OAUTH_CLIENT_ID?.trim();
...
export function getStalwartOauthClientSecret(): string | undefined {
	return process.env.STALWART_OAUTH_CLIENT_SECRET?.trim() || undefined;
}

export function getStalwartOauthIssuer(): string {
	const issuer = process.env.STALWART_OAUTH_ISSUER_URL?.trim();
...
export function getStalwartOauthRedirectUri(requestOrigin: string): string {
	const configured = process.env.STALWART_OAUTH_REDIRECT_URI?.trim();
...
export function getStalwartOauthScopes(discovery?: StalwartOauthDiscovery): string[] {
	const configured = process.env.STALWART_OAUTH_SCOPES?.trim();
