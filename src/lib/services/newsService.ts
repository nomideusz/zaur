...
const proxyUsage = {
  ...
  isRateLimited(proxy: string): boolean {
    if (!browser) return false; // Skip on server
    if (!this.timestamps.has(proxy)) return false;
  ...
  getSuggestedDelay(proxy: string): number {
    if (!browser) return 0; // Skip on server
    if (!this.timestamps.has(proxy)) return 0;
...
function parseXMLFeed(xmlText: string): any {
...
async function parseRSSFeed(feedText: string, sourceId: string): Promise<any> {
...
function processFeedItems(feed: any, source: NewsSource, sourceId: string): NewsItem[] {
  ...
  try {
    const items = feed.items.map((item: RssItem, index: number) => {
      if (!item) {
...
function generateMockNewsItems(sourceId: string, count: number = 5): NewsItem[] {
...
export async function fetchAllNews(): Promise<NewsResponse> {
  try {
    ...
    const promiseResults = await Promise.allSettled(
      newsSources.map(source => fetchNewsFromSource(source.id))
    );
    ...
    const remainingItemsBySource = new Map<string, NewsItem[]>();
    ...
    remainingItemsBySource.forEach((items, sourceId) => {
      ...
      if (sourceId.toLowerCase().includes('nature')) return;
    ...
    if (balancedItems.length === 0) {
      ...
      const mockItems = newsSources.map(source => 
        generateMockNewsItems(source.id, 2) // Reduced count for better balance
      ).flat();
    ...
    const sortedItems = balancedItems.sort((a: NewsItem, b: NewsItem) => {
      // First sort by date
    ...
    const mockItems = newsSources.map(source => 
      generateMockNewsItems(source.id, 1) // One item per source for balance
    ).flat();
...
export async function fetchNewsByCategory(category: string): Promise<NewsResponse> {
  try {
    ...
    // Create promises but handle individual failures
    const promiseResults = await Promise.allSettled(
      relevantSources.map(source => fetchNewsFromSource(source.id))
    );
    ...
    if (balancedItems.length === 0) {
      ...
      const mockItems = relevantSources.map(source => 
        generateMockNewsItems(source.id, 2)
      ).flat();
