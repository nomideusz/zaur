...
- **Mail** — inbox, folders, search (with operators like `from:` and `has:attachment`), compose, reply/forward, attachments, quick reply, undo for delete; on phones a hamburger drawer carries all navigation and a single floating New button as the primary action, with single-action swipes (Seen / Trash), long-press scrub-select, and pull-to-refresh
...
| `PUBLIC_TRACEWAY_DSN` | Optional Traceway DSN (`{token}@{url}/api/report`) for browser and server error reporting and server tracing |
