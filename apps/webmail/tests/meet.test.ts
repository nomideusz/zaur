...
describe('meet helpers', () => {
	...
	it('accepts room ids and rejects path tricks', () => {
	...
	it('recognizes webmail join URLs and legacy Galene group URLs', () => {
		assert.equal(isMeetingUrl('https://webmail.zaur.app/meet/zaur-abc123def456'), true);
		assert.equal(isMeetingUrl('https://webmail.zaur.app/meet/zaur-abc123def456/'), true);
		assert.equal(isMeetingUrl('https://meet.zaur.app/group/zaur-abc123def456/'), true);
		assert.equal(isMeetingUrl('https://meet.zaur.app/group/zaur-abc123def456'), true);
		assert.equal(isMeetingUrl('https://meet.jit.si/zaur-abc123'), false);
		assert.equal(isMeetingUrl('https://meet.zaur.app/'), false);
		assert.equal(isMeetingUrl('Studio A'), false);
	});

	it('extracts a group id from plain location text', () => {
		assert.equal(
			extractMeetingGroup('https://webmail.zaur.app/meet/zaur-room1'),
			'zaur-room1'
		);
		assert.equal(
			extractMeetingGroup('Join at https://webmail.zaur.app/meet/zaur-room1 please'),
			'zaur-room1'
		);
		assert.equal(extractMeetingGroup('https://meet.zaur.app/group/zaur-room1/'), 'zaur-room1');
		assert.equal(extractMeetingGroup('Studio A'), null);
	});
	...
	it('creates a same-origin join URL', () => {
		...
		assert.equal(isMeetingUrl(url), true);
		const group = extractMeetingGroup(url);
		assert.ok(group);
