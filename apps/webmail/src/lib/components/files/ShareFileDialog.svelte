<script lang="ts">
	...
	let email = $state('');
	let role = $state<FileShareRole>('read');
	let submitting = $state(false);
	let loadingPeople = $state(false);
	let error = $state<string | null>(null);
	let principals = $state<Record<string, JMAPPrincipal>>({});

	const item = $derived(nodeId ? files.nodeById(nodeId) : undefined);
	...
	async function changeRole(principalId: string, nextRole: FileShareRole) {
		const client = auth.client;
		const target = item;
		if (!client || !target) return;
		try {
			await files.share(client, target.id, principalId, nextRole);
		} catch (err) {
	...
	async function removeShare(principalId: string) {
		...
		if (!client || !target) return;
		try {
			await files.unshare(client, target.id, principalId);
			...
			>
				<div class="flex flex-col gap-2">
					<Dialog.Title class="text-base font-semibold text-fg">
						Share {item ? `“${item.name}”` : 'file'}
					</Dialog.Title>
					<Dialog.Description class="text-sm text-fg-muted">
						People with an account on this mail server can view or edit this
						{item?.nodeType === 'directory' ? 'folder' : 'file'}.
					</Dialog.Description>
				</div>
						...
						{#each shareEntries as [principalId, rights] (principalId)}
							<li class="flex items-center gap-2">
								...
								<SettingsSelect
									...
									value={fileShareRoleFromRights(rights)}
									options={SHARE_ROLES}
									class="w-28"
									onchange={(value) => changeRole(principalId, value as FileShareRole)}
								/>
								<button
									type="button"
				...
				<form class="flex flex-col gap-3" onsubmit={addShare}>
					...
					<SettingsSelect
						...
						value={role}
						options={SHARE_ROLES}
						onchange={(value) => (role = value as FileShareRole)}
					/>
					{#if error}
