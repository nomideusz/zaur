...
| 2 | `mail/ComposeSendSplit.svelte` | `popover` + `menu`/`radio-group` | Medium | ✅ **Done 2026-09** — hand-rolled panel + window outside-click replaced by Ark `Popover` anchored to the Send\|▾ control. Presets stay plain buttons (a form inside an ARIA `menu` is invalid); the caret is the `Popover.Trigger` (Ark only excludes triggers from outside-dismiss) with a native title instead of Ark Tooltip, and only the layout-matching instance opens the panel. |
