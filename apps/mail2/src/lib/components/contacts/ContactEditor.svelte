...
<form onsubmit={submit} class="flex h-full flex-col">
	<div class="flex items-center justify-between gap-3 border-b border-[#e2e8f0] px-6 py-3 max-md:px-4">
		<h2 class="text-[14px] font-semibold text-slate-900">{contact ? 'Edit contact' : 'New contact'}</h2>
				...
				<div class="mt-2 flex flex-col gap-2">
					{#each draft.emails as email, index (index)}
						<div class="flex items-center gap-2">
							...
							<button type="button" class="btn-tactile !size-[30px] !p-0 text-slate-500" aria-label="Remove address" onclick={() => draft.emails.splice(index, 1)} disabled={draft.emails.length === 1 && !email.address}>
				...
				<div class="mt-2 flex flex-col gap-2">
					{#each draft.phones as phone, index (index)}
						<div class="flex items-center gap-2">
							...
							<button type="button" class="btn-tactile !size-[30px] !p-0 text-slate-500" aria-label="Remove number" onclick={() => draft.phones.splice(index, 1)} disabled={draft.phones.length === 1 && !phone.number}>
								<svg class="size-3" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M4 4l8 8M12 4l-8 8" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" /></svg>
							</button>
						</div>
					{/each}
