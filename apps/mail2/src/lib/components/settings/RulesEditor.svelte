...
<section class="z-card">
				...
				{#each draft as rule, index (rule.id)}
					...
					{#if editingId === rule.id}
						...
						<div class="z-card">
							...
							<div class="z-card-body">
								<!-- Conditions -->
								<div class="flex flex-wrap items-center gap-2 text-[13px] text-[var(--z-soft)]">
									<span>If</span>
									<select
								...
								{#if issues.length > 0}
									<ul class="mt-3.5 flex flex-col gap-1">
							...
							<div class="z-card-foot justify-between">
								<label class="flex cursor-pointer items-center gap-2.5">
									<input type="checkbox" class="z-check" checked={rule.stop} onchange={(event) => patch(rule.id, { stop: event.currentTarget.checked })} />
					...
					{:else}
						...
						-->
						<div class="z-card {rule.enabled ? '' : 'bg-[var(--z-hover)]'}">
							<div class="z-card-head">
								<input
									type="checkbox"
									class="z-check"
							...
							<div class="z-card-body flex flex-col gap-[5px]">
								<div class="flex items-center gap-[7px]">
									<span class="z-caption">{rule.match === 'any' ? 'If any of' : 'If all of'}</span>
								...
								<div class="mt-[3px] flex flex-wrap items-center gap-1.5">
									<span class="z-caption">Then</span>
									{#each rule.actions as action, actionIndex (actionIndex)}
	...
	{#if !error && data?.supported && data.managed && dirty}
		<div class="z-card-foot justify-end">
			{#if problems.length > 0}
				...
				<span class="mr-auto text-[var(--z-ch-needs-ink)]">Rules that are not finished are saved but will not run.</span>
			{/if}
			<button
				type="button"
				class="btn-tactile btn-primary !h-7 !text-[12px]"
