...
{:else if !overview}
	<section class="rounded-[10px] border border-[var(--z-hairline)] bg-[var(--z-surface)] p-[18px] text-[13px] text-[var(--z-faint)]">Loading…</section>
{:else}
	...
	<section class="rounded-[10px] border border-[var(--z-hairline)] bg-[var(--z-surface)] p-[18px] shadow-[var(--z-shadow-tactile)] max-md:p-4">
		...
		<p class="mt-1.5 text-[12.5px] leading-relaxed text-[var(--z-soft)]">
	...
	<section class="rounded-[10px] border border-[var(--z-hairline)] bg-[var(--z-surface)] p-[18px] shadow-[var(--z-shadow-tactile)] max-md:p-4">
		<div class="flex items-baseline justify-between gap-3">
			...
			<span class="rounded-[4px] px-1.5 py-0.5 text-[11px] font-semibold {overview.totpEnabled ? 'bg-[var(--z-ch-confirmed-fill)] text-[var(--z-ch-confirmed-ink)]' : 'bg-[var(--z-sunken)] text-[var(--z-soft)]'}">
		...
		{#if overview.authMethod !== 'oauth'}
			<p class="mt-3 text-[12.5px] text-[var(--z-ch-needs-ink)]">
		...
		{:else if overview.totpEnabled}
			<form {...disableTotp} class="mt-3 flex flex-wrap items-end gap-2">
				<fieldset disabled={locked} class="contents">
					...
					<button type="submit" class="btn-tactile !h-[30px] !text-[var(--z-ch-discard-ink)]" disabled={disableTotp.pending > 0}>
						{disableTotp.pending > 0 ? 'Turning off…' : 'Turn off'}
					</button>
				</fieldset>
			</form>
			{#if locked}<p class="mt-2 text-[12px] text-[var(--z-faint)]">Confirm your password above first.</p>{/if}
		{:else if totpSetup}
	...
	{#each [{ kind: 'AppPassword', form: appPasswordForm, title: 'App passwords', items: overview.appPasswords, blurb: 'For mail apps that sign in with a password of their own — Thunderbird, a phone, a printer. Each one can be revoked on its own.' }, { kind: 'ApiKey', form: apiKeyForm, title: 'API keys', items: overview.apiKeys, blurb: 'For scripts and automation talking to the server directly. A key cannot change the password or manage credentials.' }] as group (group.kind)}
		...
		<section class="rounded-[10px] border border-[var(--z-hairline)] bg-[var(--z-surface)] p-[18px] shadow-[var(--z-shadow-tactile)] max-md:p-4">
			...
			<p class="mt-1.5 text-[12.5px] leading-relaxed text-[var(--z-soft)]">{group.blurb}</p>
				...
				<ul class="mt-3 divide-y divide-[var(--z-sunken)]" role="list">
					{#each group.items as item (item.id)}
						<li class="flex items-center justify-between gap-3 py-2">
							...
							<button type="button" class="btn-tactile !h-[28px] shrink-0 !text-[var(--z-ch-discard-ink)]" disabled={locked || busy === item.id} onclick={() => revoke(kind, item)}>
	...
	<section class="rounded-[10px] border border-[var(--z-hairline)] bg-[var(--z-surface)] p-[18px] shadow-[var(--z-shadow-tactile)] max-md:p-4">
		<div class="flex items-baseline justify-between gap-3">
			...
			{#if otherSessions.length}
				<button type="button" class="btn-tactile !h-[28px] !text-[var(--z-ch-discard-ink)]" disabled={locked || busy === 'others'} onclick={signOutOthers}>
