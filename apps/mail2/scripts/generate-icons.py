...
continuous stroke on a 16-unit grid, round caps and joins, in the shell's one
functional ink. The polylines below mirror the component's SVG, so the PNGs are the
...
INK = (51, 65, 85, 255)  # --z-strong


# ZaurMark.svelte's path on the 16-unit grid.
GLYPH = [(3, 3), (13, 3), (3, 13), (13, 13)]
...
def glyph(draw: ImageDraw.ImageDraw, x: float, y: float, unit: float, stroke: float, ink=None) -> None:
    ...
    pts = [(x + px * unit, y + py * unit) for px, py in GLYPH]
    fill = ink or INK
